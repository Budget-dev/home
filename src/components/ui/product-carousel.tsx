...
import { ChevronLeft, ChevronRight, Clock, Star, ShoppingCart, ArrowRight } from "lucide-react";
...
export const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  return (
    <motion.div
      ...
      onClick={handleClick}
      className="group relative w-[170px] sm:w-[210px] md:w-[240px] h-[340px] sm:h-[380px] md:h-[410px] flex-shrink-0 cursor-pointer select-none"
    >
      <div className="flex flex-col h-full overflow-hidden rounded-2xl md:rounded-3xl border border-[#E8ECE9] bg-white transition-all duration-300 hover:shadow-xl hover:border-primary/20">
        ...
        <div className="relative h-[160px] sm:h-[190px] md:h-[210px] w-full bg-[#FAF8F5] p-2 md:p-3 flex items-center justify-center overflow-hidden border-b border-[#F2ECE1] shrink-0">
          {discountText && (
            <div className="absolute left-2 top-2 z-10 rounded-full bg-primary px-2 py-0.5 text-[9px] md:text-[10px] font-black text-white shadow-xs">
              {discountText}
            </div>
          )}

          {product.badges && product.badges.length > 0 && (
            <div className="absolute right-2 top-2 z-10 rounded-full bg-[#D4A017] px-2 py-0.5 text-[8px] md:text-[9px] font-extrabold uppercase text-white shadow-xs">
              {product.badges[0]}
            </div>
          )}
        ...
        {/* Content Section */}
        <div className="flex flex-col flex-1 justify-between p-2.5 sm:p-3 md:p-4 space-y-1.5 md:space-y-2 overflow-hidden">
          <div className="space-y-1 md:space-y-1.5">
            ...
            <div className="flex items-center justify-between text-[9px] md:text-xs font-bold text-[#7A6848] h-5">
              <div className="flex items-center space-x-1 text-emerald-800 bg-emerald-50 px-1.5 py-0.5 rounded-md border border-emerald-100 shrink-0">
                ...
                <span className="text-[9px] md:text-[10px] font-extrabold uppercase">{deliveryTime}</span>
              </div>
            ...
            {/* Product Name - Line clamped with fixed height */}
            ...
            <p className="text-[10px] text-[#7A6848] font-medium truncate">{quantity}</p>
          </div>
