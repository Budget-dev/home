...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onSuccess, initialSignUp = false }) => {
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm bg-gradient-to-b from-sky-50/50 to-white rounded-3xl shadow-2xl p-8 flex flex-col items-center border border-blue-100 text-black relative animate-in zoom-in-95 duration-300">
        <button 
          onClick={onClose}
          type="button"
          className="absolute top-4 right-4 z-50 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 transition-all cursor-pointer"
        >
          <X className="w-4 h-4" />
        </button>
