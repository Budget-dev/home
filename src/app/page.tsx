        ...
        <main>
          ...
          <section className="py-4 md:py-16" id="products">
            <div className="max-w-[1400px] mx-auto px-3 sm:px-5 md:px-10">
              <div className="flex justify-center mb-4 md:mb-12 overflow-x-auto no-scrollbar px-1 w-full">
                <div className="flex gap-0.5 sm:gap-1 md:gap-2.5 items-center bg-white p-1 md:p-1.5 rounded-full border border-[#DDD0B5]/50 shadow-sm w-full max-w-[420px] sm:w-fit justify-between sm:justify-center transform-gpu translate-z-0">
                  {CATEGORIES.map((cat) => (
                    <button
                      key={cat.id}
                      onClick={() => handleCategoryFilter(cat.id)}
                      className={cn(
                        "flex items-center gap-0.5 md:gap-1.5 px-2.5 sm:px-3.5 md:px-6 py-1.5 md:py-2.5 rounded-full text-[9.5px] sm:text-[10.5px] md:text-sm font-black transition-all whitespace-nowrap shrink-0",
                        filter === cat.id 
                          ? "bg-primary text-white shadow-md" 
                          : "text-[#7A6848] hover:bg-primary/5"
                      )}
                    >
                      <span className="text-xs sm:text-sm md:text-base">{cat.ico}</span>
                      {cat.label}
                    </button>
                  ))}
                </div>
