...
export default function ProductDetailsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBFA] text-[#100C06] pb-[120px] md:pb-0">
      <Ticker />
      <Header 
          ...
          <div className="lg:col-span-6 flex flex-col justify-between space-y-3 sm:space-y-4">
            ...
            <div className="bg-[#FAF8F3] border border-[#EEE0BC] rounded-2xl p-3.5 sm:p-4 space-y-3">
                    ...
                    return (
                      <button 
                        ...
                        className={cn(
                          ...
                          isSelected 
                            ? "bg-[#1B5E3B] text-white border-[#1B5E3B] shadow-xs ring-2 ring-[#1B5E3B]/20 scale-[1.02]" 
                            : "bg-white text-[#504028] border-[#DDD0B5] hover:border-primary/60 hover:bg-[#FDFBFA]"
                        )}
                      >
                        <span className="tracking-tight">{v.s}</span>
                        <span className={cn("text-[11px] font-extrabold", isSelected ? "text-emerald-200" : "text-[#1B5E3B]")}>
              ...
              <div className="pt-2 border-t border-[#EEE0BC]/70 flex items-center justify-between">
                ...
                <div className="flex items-center bg-white border border-[#DDD0B5] rounded-full h-9 px-1.5 shadow-2xs">
                  <button 
                    type="button"
                    onClick={() => setQty(q => Math.max(1, q - 1))} 
                    className="w-7 h-7 flex items-center justify-center text-[#1B5E3B] hover:bg-[#F1EAD8] rounded-full transition-colors active:scale-90"
      ...
      {/* SINGLE STICKY BOTTOM ACTION BAR FOR MOBILE (<md) - NO DOUBLE BUTTONS, PERFECT LAYOUT */}
      <div className="fixed bottom-[56px] left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#EEE0BC] px-3 py-2.5 shadow-2xl md:hidden flex items-center justify-between gap-2">
        <div className="flex flex-col justify-center shrink-0 min-w-[85px]">
          <span className="text-[9.5px] font-bold text-[#7A6848] uppercase tracking-tight truncate max-w-[95px]">
        ...
        <div className="flex items-center gap-1.5 flex-1 max-w-[230px]">
          <Button 
            onClick={() => { addToCart({ ...product, price: displayPrice, vol: selectedSize } as any, qty); setIsCartOpen(true); }}
            className="flex-1 h-9 bg-white border border-primary text-primary hover:bg-primary/5 font-extrabold uppercase text-[11px] rounded-lg px-1"
          >
            Add
          </Button>
          <Button 
            onClick={handleBuyNow}
            className="flex-1.2 h-9 bg-primary text-white hover:bg-primary/95 font-extrabold uppercase text-[11px] rounded-lg shadow-sm px-1 whitespace-nowrap"
          >
            Buy Now ✦
          </Button>
        </div>
